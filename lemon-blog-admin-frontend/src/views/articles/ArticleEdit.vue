    ...
    getArticle(this.articleId).then(data => {
      this.$nextTick(() => {
        ...
        this.form.setFieldsValue({
          ...
          status: data.status,
          createDate: moment(data.createdAt.split(/\s+/)[0], this.dateFormat),
          createTime: moment(data.createdAt.split(/\s+/)[1], this.timeFormat),
    ...
    handleSubmit (e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          updateArticle(this.articleId, params).then(() => {
